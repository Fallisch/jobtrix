...
test.describe("Issue #43 – PDF-Layout 'Kreativ' in Bewerbungshistorie", () => {
  test("Re-Export aus der Bewerbungshistorie behält Layout 'Kreativ' und Akzentfarbe bei", async ({ page }) => {
    ...
    // Akzentfarbe wird in der Kreativ-Sidebar (creative-sidebar) als
    // Hintergrundfarbe für CV und Anschreiben verwendet.
    expect(cvContent).toContain(fillOperator);
    expect(letterContent).toContain(fillOperator);

    // CV-Stil "classic": Berufserfahrung und Ausbildung chronologisch.
    const cvText = extractText(cvContent);
    expect(cvText.indexOf("Firma A")).toBeGreaterThanOrEqual(0);
    expect(cvText.indexOf("Firma B")).toBeGreaterThan(cvText.indexOf("Firma A"));
    expect(cvText.indexOf("FU Berlin")).toBeGreaterThanOrEqual(0);
    expect(cvText.indexOf("HU Berlin")).toBeGreaterThan(cvText.indexOf("FU Berlin"));
