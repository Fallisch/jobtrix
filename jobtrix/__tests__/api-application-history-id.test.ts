...
import { DELETE, GET } from "@/app/api/application-history/[id]/route";
...
describe("DELETE /api/application-history/[id]", () => {
  const email = `app-history-delete-test-${Date.now()}@example.com`;
  let userId: string;
  ...
  it("liefert 401 ohne Session", async () => {
    ...
    const res = await DELETE(new Request("http://localhost"), makeContext("any-id"));

    expect(res.status).toBe(401);
  });

  it("löscht den Eintrag des eingeloggten Users", async () => {
    const entryId = await createEntry();
    mockedGetServerSession.mockResolvedValue({ user: { id: userId }, expires: "" });

    const res = await DELETE(new Request("http://localhost"), makeContext(entryId));

    expect(res.status).toBe(200);
    const deleted = await prisma.applicationHistoryEntry.findUnique({ where: { id: entryId } });
  ...
  it("liefert 404 für eine nicht existierende ID", async () => {
    ...
    const res = await DELETE(new Request("http://localhost"), makeContext("nonexistent-id"));

    expect(res.status).toBe(404);
  });

  it("liefert 404 für einen Eintrag eines anderen Nutzerkontos und löscht ihn nicht", async () => {
    const entryId = await createEntry();
    const otherEmail = `app-history-delete-test-other-${Date.now()}@example.com`;
    const otherPasswordHash = await bcrypt.hash("password", 10);
    const otherUser = await prisma.user.create({ data: { email: otherEmail, passwordHash: otherPasswordHash } });
    ...
    try {
      ...
      expect(res.status).toBe(404);
      const stillExists = await prisma.applicationHistoryEntry.findUnique({ where: { id: entryId } });
